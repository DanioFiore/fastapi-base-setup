...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, redis_client: Optional[redis.Redis] = None):
        ...
        self.endpoint_limits = {
            ...
            "/api/auth/forgot-password": {"requests_per_minute": 2, "requests_per_hour": 5},
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> JSONResponse:
        ...
        # Check rate limits
        rate_limit_result = await self._check_rate_limits(client_id, request.url.path, limits)
    ...
    def _create_rate_limit_response(self, rate_limit_result: dict) -> JSONResponse:
        ...
        headers = {
            ...
            "X-RateLimit-Remaining-Minute": str(max(0, rate_limit_result["minute_limit"] - rate_limit_result["minute_count"])),
            "X-RateLimit-Reset-Minute": str(rate_limit_result["reset_time_minute"]),
            "X-RateLimit-Limit-Hour": str(rate_limit_result["hour_limit"]),
            "X-RateLimit-Remaining-Hour": str(max(0, rate_limit_result["hour_limit"] - rate_limit_result["hour_count"])),
    ...
    def _add_rate_limit_headers(self, response, rate_limit_result: dict) -> None:
        ...
        response.headers["X-RateLimit-Remaining-Minute"] = str(max(0, rate_limit_result["minute_limit"] - rate_limit_result["minute_count"]))
        response.headers["X-RateLimit-Reset-Minute"] = str(rate_limit_result["reset_time_minute"])
        response.headers["X-RateLimit-Limit-Hour"] = str(rate_limit_result["hour_limit"])
        response.headers["X-RateLimit-Remaining-Hour"] = str(max(0, rate_limit_result["hour_limit"] - rate_limit_result["hour_count"]))
        response.headers["X-RateLimit-Reset-Hour"] = str(rate_limit_result["reset_time_hour"])


def add_rate_limiting_middleware(app: FastAPI, redis_client: Optional[redis.Redis] = None) -> None:
    """
